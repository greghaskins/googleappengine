...
import com.google.appengine.api.servers.dev.LocalServerController;
 ...
 *
 */
public class BackgroundServers implements BackgroundContainer, LocalServerController {
  ...
  private enum BackgroundServerState {
  ...
  private static BackgroundServers instance = new BackgroundServers();

  public static BackgroundServers getInstance() {
    return instance;
  }
  ...
  private Map<BackgroundServers.ServerInstanceEntry, ServerWrapper> backendServers =
  ...
  private Logger logger = Logger.getLogger(BackgroundServers.class.getName());
  ...
  /**
   * Shutdown all background servers
   *
   * @throws Exception
   */
  @Override
   ...
   */
  @Override
  public TreeMap<String, ServerStateInfo> getServerState(String requestHostName) {
    TreeMap<String, ServerStateInfo> serverInfoMap = new TreeMap<String, ServerStateInfo>();
    for (ServerWrapper serverWrapper : backendServers.values()) {
      ...
      ServerStateInfo ssi = serverInfoMap.get(name);
      if (ssi == null) {
        ssi = new ServerStateInfo(serverWrapper.serverEntry);
  ...
  @Override
  public synchronized void startServer(String serverToStart) throws IllegalStateException {
    ...
    for (ServerWrapper server : backendServers.values()) {
      if (server.getName().equals(serverToStart)) {
        if (server.getState() != BackgroundServerState.STOPPED) {
          continue;
        }
        if (server.isLoadBalanceServer()) {
          server.compareAndSetServerState(
              BackgroundServerState.RUNNING, BackgroundServerState.STOPPED);
          continue;
        }
        server.compareAndSetServerState(
            BackgroundServerState.SLEEPING, BackgroundServerState.STOPPED);
  ...
  @Override
  public synchronized void stopServer(String serverToStop) throws IllegalStateException, Exception {
    ...
    for (ServerWrapper server : backendServers.values()) {
      if (server.getName().equals(serverToStop)) {
        ...
        if (server.isLoadBalanceServer()) {
          server.compareAndSetServerState(
              BackgroundServerState.STOPPED, BackgroundServerState.RUNNING);
          continue;
        }
  ...
  /**
   * Start all background servers, the number of servers to start is specified
  ...
  public void startupAll(BackendsXml backendsXml) throws Exception {
    if (backendsXml == null) {
      logger.warning("Got null backendsXml config.");
      return;
    }
    ...
    Map<BackgroundServers.ServerInstanceEntry, ServerWrapper> serverMap = Maps.newHashMap();
  ...
  private BackendsXml.Entry resolveDefaults(BackendsXml.Entry entry) {
    return new BackendsXml.Entry(
        ...
        entry.getAccess() == null ? BackendsXml.AccessType.PRIVATE : entry.getAccess(),
        entry.getMaxConcurrentRequests() == null ? DEFAULT_MAX_CONCURRENT_REQUESTS :
                                                   entry.getMaxConcurrentRequests(),
        entry.getOptions(),
        entry.getState() == null ? BackendsXml.State.STOPPED : entry.getState());
  }
  ...
  void forwardToServer(String requestedServer, int instance, HttpServletRequest hrequest,
    ...
    RequestDispatcher requestDispatcher =
        jettyContext.getServletContext().getRequestDispatcher(hrequest.getServletPath());
  ...
  boolean acquireServingPermit(
    ...
    try {
      ...
      synchronized (server) {
        ...
        if (server.getState() == BackgroundServerState.SLEEPING) {
          logger.finest(server + ": waking up sleeping server");
          server.sendStartRequest();
        }

        if (server.getState() == BackgroundServerState.RUNNING_START_REQUEST) {
  ...
  private class ServerWrapper {
    ...
    private BackgroundServerState serverState = BackgroundServerState.SHUTDOWN;
      ...
      try {
        ...
        try {
          ...
          if (returnCode >= 500) {
            ...
            logger.fine(
                String.format("background server %d.%s request to /_ah/start completed, code=%d",
                    serverInstance, serverEntry.getName(), returnCode));
            compareAndSetServerState(
                BackgroundServerState.RUNNING, BackgroundServerState.RUNNING_START_REQUEST);
    ...
    public void shutdown() throws Exception {
      synchronized (ServerWrapper.this) {
        if (serverState == BackgroundServerState.RUNNING
            || serverState == BackgroundServerState.RUNNING_START_REQUEST) {
          triggerLifecycleShutdownHook();
        }
        container.shutdown();
        serverState = BackgroundServerState.SHUTDOWN;
      }
    }

    void startup(boolean setStateToStopped) throws Exception {
      compareAndSetServerState(BackgroundServerState.INITIALIZING, BackgroundServerState.SHUTDOWN);
      ...
      if (setStateToStopped) {
        compareAndSetServerState(BackgroundServerState.STOPPED, BackgroundServerState.INITIALIZING);
      } else {
        logger.info(
        ...
        if (isLoadBalanceServer()) {
          compareAndSetServerState(
              BackgroundServerState.RUNNING, BackgroundServerState.INITIALIZING);
        } else {
          compareAndSetServerState(
              BackgroundServerState.SLEEPING, BackgroundServerState.INITIALIZING);
        }
      }
    }
    ...
    void sendStartRequest() {
      compareAndSetServerState(
          BackgroundServerState.RUNNING_START_REQUEST, BackgroundServerState.SLEEPING);
      if (serverInstance >= 0) {
        ...
        requestThread.setName(
            "BackgroundServersStartRequestThread." + serverInstance + "." + serverEntry.getName());
        requestThread.start();
      }
    }
    ...
    boolean acceptsConnections() {
      synchronized (ServerWrapper.this) {
        return (serverState == BackgroundServerState.RUNNING
            || serverState == BackgroundServerState.RUNNING_START_REQUEST
            || serverState == BackgroundServerState.SLEEPING);
      }
    }
    ...
    private void compareAndSetServerState(
        BackgroundServerState newState, BackgroundServerState... acceptablePreviousStates)
        throws IllegalStateException {
      synchronized (ServerWrapper.this) {
        for (BackgroundServerState acceptableStates : acceptablePreviousStates) {
    ...
    int getMaxPendingQueueSize() {
      return serverEntry.hasOption(BackendsXml.Option.FAIL_FAST) ? 0 : MAX_PENDING_QUEUE_LENGTH;
    }
    ...
    BackgroundServerState getState() {
      synchronized (ServerWrapper.this) {
        return serverState;
      }
    }
