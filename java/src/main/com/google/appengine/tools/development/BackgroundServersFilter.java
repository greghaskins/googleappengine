...
import com.google.appengine.api.servers.ServerService;
 ...
 *
 */
public class BackgroundServersFilter implements Filter {
  ...
  private final BackgroundServers backgroundServersManager;

  private Logger logger = Logger.getLogger(BackgroundServersFilter.class.getName());

  @VisibleForTesting
  BackgroundServersFilter(BackgroundServers backgroundServers) {
    this.backgroundServersManager = backgroundServers;
  }

  public BackgroundServersFilter() {
    this.backgroundServersManager = BackgroundServers.getInstance();
  }
  ...
  RequestType getRequestType(HttpServletRequest hrequest) {
    ...
    String directServerName = backgroundServersManager.getServerNameFromPort(serverPort);

    if (hrequest.getRequestURI().equals("/_ah/start") && directServerName != null) {
      return RequestType.SERVER_STARTUP_REQUEST;
    } else if (hrequest.getAttribute(ServerService.REQUEST_HEADER_SERVER_REDIRECT) != null) {
      return RequestType.REDIRECTED_SERVER_REQUEST;
    } else if (directServerName != null) {
      int directServerReplica = backgroundServersManager.getServerInstanceFromPort(serverPort);
      ...
      String serverRedirectHeader =
          getHeaderOrParameter(hrequest, ServerService.REQUEST_HEADER_SERVER_REDIRECT);
  ...
  private boolean instanceAcceptsConnections(
    ...
    if (!backgroundServersManager.checkInstanceExists(requestedServer, instance)) {
      String msg =
    ...
    if (backgroundServersManager.checkInstanceStopped(requestedServer, instance)) {
      String msg =
    ...
    if (!backgroundServersManager.acquireServingPermit(requestedServer, instance, true)) {
  ...
  private void doServerRedirect(HttpServletRequest hrequest, HttpServletResponse hresponse)
    ...
    String requestedServer =
        backgroundServersManager.getServerNameFromPort(hrequest.getServerPort());
    ...
    if (instance != -1) {
      ...
      if (!backgroundServersManager.checkServerExists(requestedServer)) {
        String msg = String.format("Got request to non-configured server: %s", requestedServer);
        logger.warning(msg);
        hresponse.sendError(HttpServletResponse.SC_BAD_GATEWAY, msg);
        return;
      }
      if (backgroundServersManager.checkServerStopped(requestedServer)) {
        String msg = String.format("Got request to stopped server: %s", requestedServer);
        logger.warning(msg);
        hresponse.sendError(SERVER_STOPPED_ERROR_CODE, msg);
        return;
      }
      instance = backgroundServersManager.getAndReserveFreeInstance(requestedServer);
      if (instance == -1) {
    ...
    try {
      ...
      hrequest.setAttribute(ServerService.REQUEST_HEADER_SERVER_REDIRECT, requestedServer);
      hrequest.setAttribute(
          ServerService.REQUEST_HEADER_INSTANCE_REDIRECT, Integer.valueOf(instance));
      backgroundServersManager.forwardToServer(requestedServer, instance, hrequest, hresponse);
    } finally {
      backgroundServersManager.returnServingPermit(requestedServer, instance);
    }
  }
  ...
  private void doDirectServerRequest(
      ...
      throws IOException, ServletException {
    int serverPort = hrequest.getServerPort();
    String requestedServer = backgroundServersManager.getServerNameFromPort(serverPort);
    int instance = backgroundServersManager.getServerInstanceFromPort(serverPort);
  ...
  private void doRedirectedServerRequest(
      HttpServletRequest hrequest, HttpServletResponse hresponse, FilterChain chain)
      throws IOException, ServletException {
    String backgroundServer =
        (String) hrequest.getAttribute(ServerService.REQUEST_HEADER_SERVER_REDIRECT);
    Integer instance =
        (Integer) hrequest.getAttribute(ServerService.REQUEST_HEADER_INSTANCE_REDIRECT);
    logger.finest("redirected request to server instance: " + instance + "." + backgroundServer);

    injectServerApiInfo(backgroundServer, instance);
    chain.doFilter(hrequest, hresponse);
  }
  ...
  private void doStartupRequest(
      ...
      throws IOException, ServletException {
    int serverPort = hrequest.getServerPort();
    String backgroundServer = backgroundServersManager.getServerNameFromPort(serverPort);
    int instance = backgroundServersManager.getServerInstanceFromPort(serverPort);
    logger.finest("startup request to: " + instance + "." + backgroundServer);
    injectServerApiInfo(backgroundServer, instance);
    chain.doFilter(hrequest, hresponse);
  }
  ...
  private void injectServerApiInfo(String currentServer, int instance) {
    ...
    threadLocalAttributes.put(ServerService.INSTANCE_ID_ENV_ATTRIBUTE, instance + "");
    if (currentServer != null) {
      threadLocalAttributes.put(ServerService.SERVER_ID_ENV_ATTRIBUTE, currentServer);
    }
    Map<String, String> portMapping = backgroundServersManager.getPortMapping();
    threadLocalAttributes.put(
        ServerService.DEVAPPSERVER_PORTMAPPING_KEY, portMapping);
    if (portMapping.size() > 0) {
      threadLocalAttributes.put(
          LocalServerController.SERVER_CONTROLLER_ATTRIBUTE_KEY, backgroundServersManager);
    }
  }
  ...
  static int getInstanceIdFromRequest(HttpServletRequest request) {
    try {
      return Integer.parseInt(
          getHeaderOrParameter(request, ServerService.REQUEST_HEADER_INSTANCE_REDIRECT));
