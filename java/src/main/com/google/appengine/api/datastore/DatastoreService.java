 ...
 *
 * The {@code DatastoreService} can be used transactionally and supports the
 * notion of a "current" transaction.  A current transaction is established by
 * calling {@link #beginTransaction()}.  The transaction returned by this method
 * ceases to be current when an attempt is made to commit or rollback or when
 * another call is made to {@link #beginTransaction()}.  A transaction can only
 ...
 * {@code null}) {@link Transaction} to these methods or relying on the
 * behavior governed by the {@link ImplicitTransactionManagementPolicy}.
 * If a user explicitly provides a {@link Transaction} it is up to the user to
 * call {@link Transaction#commit()} or {@link Transaction#rollback()} at the
 ...
 * transactions according to {@link ImplicitTransactionManagementPolicy} are:
 * {@link #delete(Key...)}, {@link #delete(Iterable)}, {@link #get(Key)},
 * {@link #get(Iterable)}, {@link #put(Entity)}, and {@link #put(Iterable)}.
 *
 * The overload of prepare that takes a {@link Transaction} parameter
 * behaves the same as the overloads of put, get, and delete that take a
 * {@link Transaction} parameter.  However, the overload of prepare that does
 * not take a {@link Transaction} parameter, unlike put, get, and delete, does
 * not use an existing {@link Transaction} if one is already running and does
 * not consult the {@link ImplicitTransactionManagementPolicy} if one is not
 * already running.
 *
 */
