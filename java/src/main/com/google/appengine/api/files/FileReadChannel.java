...
/**
 * An {@code ReadableByteChannel} for reading bytes from an
 * {@link AppEngineFile}.
 * <p>
 * A {@code FileReadChannel} has a current <i>position</i> within its file which
 * can be both {@link #position() queried} and {@link #position(long) modified}.
 * <p>
 * An instance of {@code FileReadChannel} is obtained via the method
 * {@link FileService#openReadChannel(AppEngineFile, boolean)}.
 * <p>
 * A {@code FileReadChannel} is associated with a single App Engine request and
 * may not be used outside of the request in which it is constructed. Therefore
 * an instance of {@code FileReadChannel} should not be cached between requests.
 * Instead, {@link #close() close } the channel at the end of the request, cache
 * the {@code AppEngineFile} or just the {@link AppEngineFile#getFullPath()
 * path}, and create a new {@code FileReadChannel} in a later request.
 * <p>
 * When a {@code FileReadChannel} is constructed the underlying file may
 * optionally be <b>locked</b>. Successful aquisition of the lock means that no
 ...
 * {@link #close()} is invoked. When the request terminates, {@code close()}
